...
\item \textbf{Causal evidence}: Frozen do-interventions and orthogonal factorial confirm nearest-class dominance; cross-modal dose-response (6 unseen archs): 6/6 directional PASS.
...
\item \textbf{Expanded pre-registered holdout (H8+)}: 8 unseen models $\times$ 8 datasets ($n=54$), all parameters frozen. All 6 pre-registered criteria pass ($r=0.859$, MAE$=0.081$). Pointwise MAE is close to the dataset-mean baseline, confirming mechanistic ordering rather than high-precision calibration.
\end{enumerate}
...
Measuring $\rho$ empirically on five architectures (Pythia-160m to OLMo-1B) and three datasets ($K=4,14,77$): $\rho\in[0.41,0.49]$, $d_\mathrm{eff,comp}\in[1.69,1.97]$, with CV$=3.9\%$ across architectures (tighter than $\alpha$'s CV$=2.3\%$).
The near-simplex value $\rho\approx0.45$ ($10\%$ below perfect NC) predicts $\alpha\approx1.525$ vs.\@ observed $1.477$ ($3.2\%$ error, zero free parameters).
This provides a geometric explanation for $\alpha$ universality: class centroids approximately form a regular simplex in whitened embedding space (near-NC geometry), so the competition equicorrelation $\rho$ is approximately architecture-universal.
...
This decouples the universality of near-simplex geometry from the within-decoder $\alpha$ constancy: $\rho\approx0.45$ is a global structural property of categorical representation; the additional factor that shifts $\alpha$ across families (decoder $\approx1.5$, encoder $\approx7$, ViT $\approx0.6$) is not captured by the current $\rho$-based formula.
...
\paragraph{H8+: Expanded pre-registered holdout (8 models $\times$ 8 datasets).}
A factorized prospective test freezes all CTI parameters---family-specific $\alpha$, per-dataset $C_d$, per-family $C_f$, and $\gamma$ (model-size term)---from the 20-model training set (pre-registered at commit \texttt{719da25}) and predicts $q_\mathrm{norm}$ for 8 unseen architectures across 8 datasets ($n=54$ valid predictions after floor/ceiling exclusion at $q\leq0.01$ or $q\geq0.99$; \texttt{results/cti\_utility\_revised.json}).
All six pre-registered criteria pass:
Pearson $r(\mathrm{logit})=0.859$ (H8a: $\geq0.85$),
MAE$=0.081$ (H8b: $\leq0.10$),
87\% within 0.15 (H8c: $\geq80\%$),
per-family $r$: encoder$=0.948$, decoder$=0.806$ (H8d: $\geq0.70$),
mean signed error$=+0.038$ (H8e: $\in[-0.05,+0.05]$),
outlier rate$=5.6\%$ (H8f: $\leq10\%$).

\emph{Sensitivity analysis.}
Restricting to the moderate regime $q\in[0.05,0.95]$ improves $r$ to 0.907 ($n=52$); excluding two pathological models (bloom-560m, near-chance on 4/8 datasets; gemma-3-1b, catastrophic on DBpedia) yields $r=0.946$ ($n=46$, MAE$=0.066$).
Pointwise MAE is close to the dataset-mean baseline (CTI 0.081 vs.\ baseline 0.084), confirming that the law captures \emph{mechanistic ordering} rather than high-precision calibration.
CTI's unique value emerges in within-dataset architecture ranking: on the four datasets with meaningful cross-architecture variation (spread${}>0.20$), CTI achieves mean Spearman $\rho=0.698$ (20newsgroups: $0.929$, Amazon Massive: $0.826$, DBpedia: $0.607$, Banking77: $0.429$), while the dataset-mean baseline provides $\rho=0$ by definition.
...
\caption{H8+ expanded holdout: predicted vs.\ actual $q_\mathrm{norm}$ for 8 unseen architectures across 8 datasets ($n=54$). All parameters frozen from 20-model training set. Three annotated outliers account for all errors ${>}0.20$.}
\label{fig:h8plus}
\end{figure}
...
\textbf{A1 universal pass (5/5)}: r-scale $\geq 0.976$ for all architectures (pythia-160m: $0.986$, BERT-base: $0.961$, ELECTRA-small: $0.992$, pythia-410m: $0.991$, RWKV-4: $0.976$).
...
Four additional datasets (Yahoo $K=10$; Language ID $K=20$; News Categories $K=42$; Amazon Massive $K=59$) complete the 6-dataset prospective picture:
\begin{center}
\begin{tabular}{llrrr}
\toprule
...
Macaque IT \citep{cadieu2014deep} ($K=7$): per-image $r=0.41$ ($p<10^{-4}$); class-level underpowered.
Mouse V1 \citep{stringer2018spontaneous} ($K=11$): per-image $r=0.64$ ($p<10^{-4}$); class-level confounded by size imbalance.
...
Exploratory per-area: combined VC ($r=0.869$) $>$ VISam ($0.802$) $>$ VISl ($0.774$) $>$ VISp ($0.707$) $>$ LP thalamus ($0.519$) --- law compliance increases up the visual hierarchy.
...
This suggests the near-simplex geometry of categorical representations is not a gradient training artifact but a structural property common to biological and artificial representational systems. See Figure~\ref{fig:allen-bio}.
...
Cross-arch A1 scaling (5 archs, 3 families) & r-scale $\geq 0.976$ (5/5) & PASS \\
...
\begin{table}[h]
\centering
\caption{H8+ per-model holdout summary. All parameters frozen from 20-model training set (pre-registered commit \texttt{719da25}). $n$ is the number of valid dataset predictions per model after floor/ceiling exclusion.}
...
stablelm-3b-4e1t & decoder & 8 & 0.070 & 0.156 & Good \\
opt-125m & decoder & 7 & 0.071 & 0.149 & Good \\
pythia-2.8b & decoder & 8 & 0.082 & 0.173 & Good \\
bloom-560m & decoder & 4 & 0.159 & 0.314 & Weak \\
gemma-3-1b & decoder & 4 & 0.167 & 0.532 & Weak \\
\midrule
\multicolumn{2}{l}{All (full H8+)} & 54 & 0.081 & 0.532 & PASS \\
\multicolumn{2}{l}{Excl.\ bloom+gemma} & 46 & 0.066 & 0.309 & --- \\
\multicolumn{2}{l}{$q\in[0.05,0.95]$} & 52 & 0.076 & 0.532 & --- \\
\bottomrule
\end{tabular}
\end{table}

bloom-560m operates near chance on 4/8 datasets (all excluded by floor filter or yielding large errors);
gemma-3-1b catastrophically mispredicts DBpedia ($\mathrm{err}=0.532$, predicted $q=0.679$, actual $q=0.148$).
These two models account for all outliers $>0.20$. Excluding them yields
$r=0.946$, MAE$=0.066$ ($n=46$). The pre-registered test reports the full set; the sensitivity rows are exploratory.
