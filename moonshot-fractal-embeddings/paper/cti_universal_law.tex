...
The slope constant $A$ depends on the number of classes $K$: fitting $A_{\mathrm{single}}=C - a\log K$ to the four NLP datasets ($K=4,14,20,28$) yields $C=26.10$, $a=7.78$, $r=-0.979$ ($p=0.021$). Applied prospectively to vision (CIFAR-10, $K=10$)---not used in calibration---the formula predicts $A_{\mathrm{pred}}=8.19$, vs.\ observed $A_{\mathrm{obs}}=7.51$ (ViT-Base) and $8.57$ (ViT-Large), mean relative error $6.8\%$. Combining NLP and ViT: $r(\log K, A)=-0.978$, $p=0.0007$ ($n=6$). The $K$-dependence explains why direct $d_{\mathrm{eff}}$ renormalization fails: geometric measurement of $d_{\mathrm{eff}}=\mathrm{tr}(\Sigma_W)/\sigma_{\mathrm{dir}}^2$ from embeddings gives $d_{\mathrm{eff},\mathrm{NLP}}\approx71.9$ and $d_{\mathrm{eff},\mathrm{ViT}}\approx64.5$ (too similar to explain the alpha difference); $A_{\mathrm{renorm}}=\alpha/\sqrt{d_\mathrm{eff}}$ has $\mathrm{CV}=0.82$ across architectures (fails universality). The value $d_\mathrm{eff}\approx1.71$ cited in earlier derivations was back-calculated from the empirical $\alpha$ via $(\alpha/\sqrt{4/\pi})^2$ and is not independently measured. The asymptotic prediction $\beta_\infty=1.0$ is renormalized to $\beta\approx0.746$ by finite-sample effects and anisotropic geometry (single-intercept fit; per-dataset fit gives $\hat\beta=0.326$).
...
ViT-Large at $K=10$ reaches $R^2=0.964$, matching the NLP fit in explained variance. The slope $\alpha_{\mathrm{ViT}}\approx0.63$ (in the $A\cdot\kappa\cdot\sqrt{d_\mathrm{eff}}$ form) and $\alpha_{\mathrm{NLP}}=1.477$ are from different normalizations, but when both are expressed in simple-kappa form the difference is predicted by $K$: the NLP-calibrated formula $A_{\mathrm{single}}=26.10-7.78\log K$ gives $A_{\mathrm{pred}}(K=10)=8.19$, matching ViT-Base/Large at $6.8\%$ mean relative error. Direct geometric measurement confirms $d_{\mathrm{eff}}$ is similar across modalities ($d_{\mathrm{eff},\mathrm{NLP}}\approx71.9$, $d_{\mathrm{eff},\mathrm{ViT}}\approx64.5$); the apparent alpha difference is a $K$ effect, not a modality effect.\footnote{The ViT cross-modality experiment fits $\logit(q)=A\cdot\kappa\cdot\sqrt{d_\mathrm{eff}}+C$, while the NLP law uses $\logit(q)=\alpha\cdot\kappa-\beta\log(K-1)+C_0$ with $\alpha$ absorbing $\sqrt{d_\mathrm{eff}}$. The reported $A_{\mathrm{ViT}}=0.63$ and $\alpha_{\mathrm{NLP}}=1.477$ are not directly commensurable; what is universal is the functional form (logit-linear in $\kappa$). When both are compared in single-intercept simple-kappa form they fall on the same $A=26.10-7.78\log K$ curve ($r=-0.978$, $p=0.0007$, $n=6$ NLP+ViT).}
