...
\paragraph{Direct centroid-geometry measurement.}
...
where $\alpha_\mathrm{univ}=3.598$. This gives a geometric estimate of the effective competitor count.
Results: DBpedia slope $=0.934\pm0.005$ ($r=0.9999$); Banking77 ($K=4$--$64$ subsets)
slope $=0.918\pm0.012$ ($r=0.9999$). Both slopes are near 1, suggesting
$N_\mathrm{eff}\approx(K-1)^{0.93}$ from pure centroid geometry.

This does \emph{not} contradict the regression-based $\beta=0.478$. The geometric $N_\mathrm{eff}$
exponent $\approx0.93$ reflects competition within a single (model, dataset) pair with pythia-160m.
The regression $\beta=0.478$ reflects how $\log(K-1)$ affects $\logit(q)$ \emph{across datasets}
with different intercepts $C_0$. Cross-dataset variation in $C_0$, finite-sample effects,
and approximation error in the Gumbel race model together produce the global $\beta\approx0.5$,
which may differ from the within-architecture geometric exponent.
This is an important distinction: the regression $\beta$ is a fitting parameter that compresses
multi-dataset $K$-variation, not a direct geometric observable.
Full mechanistic reconciliation of the two $\beta$ estimates requires per-model,
multi-$K$ embedding extraction across varied datasets --- a planned follow-up.
