...
Causal evidence from frozen do-interventions and an orthogonal factorial test identifies nearest-class geometry as the primary driver (farthest-class control $r=0.000$ in the orthogonal factorial; non-zero $r=0.569$ in frozen-centroid surgery reflects $\sigma_W$ leakage, not a competing cause).
...
\item \textbf{K-scaling and practical prediction}: The slope $A(K)=1.164/\log(K)+1.301$ ($r_\mathrm{fit}=0.93$). MAE$\leq$0.08 on \emph{all} six prospective datasets (pre-registered, never seen in fitting). Architecture-ranking ($\rho\geq0.85$) is non-monotone in $K$: passes at $K=20$ (language ID, $\rho=0.906$) and $K\geq59$ (intent/banking, $\rho\geq0.89$), fails at $K=42$ (news categories, $\rho=0.80$). True predictor is between-architecture $\kappa_\mathrm{nearest}$ spread, not $K$ alone: pre-registered regression on $n=10$ datasets confirms $\hat\beta_\mathrm{spread}=0.83\gg\hat\beta_{\log K}=0.05$ (spread $17\times$ more predictive; Spearman $r=0.661$, $p=0.038$; \texttt{results/cti\_spread\_vs\_K\_n10.json}).
\end{enumerate}
...
\paragraph{Kappa-spread vs.\ $K$ as predictors of ranking reliability (pre-registered, confirmed $n=10$).}
...
Initial result (\texttt{results/cti\_spread\_vs\_K.json}, $n=6$ datasets): $\hat\beta_1=0.44$, $\hat\beta_2=0.08$; H\textsubscript{K\_null} and H\textsubscript{spread} PASS; Spearman $r(\mathrm{spread},\rho)=0.886 > r(\log K,\rho)=0.600$, but Pearson H\textsubscript{spread\_r} fails (underpowered, $p\approx0.08$--$0.18$). A pre-registered follow-up extending to all available datasets (Session 64, \texttt{results/cti\_spread\_vs\_K\_n10.json}, $n=10$ datasets including 4 primary training sets) confirms all three hypotheses: $\hat\beta_1=0.83$, $\hat\beta_2=0.05$ (spread is $17\times$ more important); Spearman $r(\mathrm{spread},\rho)=0.661$ ($p=0.038$) vs.\ $r(\log K,\rho)=0.176$ ($p=0.63$); H\textsubscript{spread}: PASS ($\beta_1>0$, partial $r=0.34>0$); H\textsubscript{K\_null}: PASS ($|\hat\beta_1|\gg|\hat\beta_2|$); H\textsubscript{spread\_r}: \textbf{PASS} ($|r_\mathrm{spread}|>|r_{\log K}|$, $p=0.038$). The $p=0.038$ significance confirms the kappa-spread effect is not a small-sample artifact. \textit{Interpretation}: architecture-ranking reliability is mechanistically linked to between-architecture kappa spread. Datasets where all architectures produce similar kappa (news categories: spread$=0.051$, 20newsgroups: spread$=0.036$) yield unreliable ranking regardless of $K$; datasets with high kappa variation (language ID: spread$=0.311$, DBpedia: spread$=0.139$, AG News: spread$=0.076$) yield reliable ranking ($\rho\geq0.85$). MAE universality is robust across all datasets; ranking universality depends on kappa spread, for which $K$ is a noisy proxy ($p=0.63$ for $K$ alone).
...
We tested whether the law \emph{form} extends to biological neural systems using three independently collected multi-electrode datasets (all post-hoc exploratory; pre-registration commit \texttt{bddec1d}).
For macaque IT cortex \citep{cadieu2014deep} ($K=7$ categories, 168 multiunits, $n=1{,}960$ images): per-image Pearson $r(\text{margin}, \text{correct})=0.41$ ($p<10^{-4}$). The class-level test is underpowered at $K=7$ ($r=0.19$).
For mouse V1 \citep{stringer2018spontaneous} ($K=11$ natural image categories, 10{,}079 neurons, $n=4{,}914$): per-image $r=0.64$ ($p<10^{-4}$); class-level $r=-0.48$ is confounded by class-size imbalance.
For mouse visual cortex \citep{siegle2021survey} (Allen Neuropixels, DANDI:000021, $K=118$ natural images, $n=5{,}900$, 50 trials/image): we ran the identical protocol on all 32 independent recording sessions from the complete dataset (32 different mice). 30/32 sessions satisfy the pre-registered H1 criterion ($r(\kappa_{\mathrm{nearest}}, \logit q)>0.50$); \textbf{all 32 show positive $r$} (range $[0.44, 0.89]$, significant at $p<0.001$ in every session), mean$=0.74$, inter-session CV$=15.4\%$; per-image $r\in[0.23, 0.77]$, mean$=0.46$ (\texttt{results/cti\_allen\_all\_sessions\_complete.json}). The two non-passing sessions ($r=0.44$, $r=0.46$) remain individually significant ($p<0.001$, $n=118$); they correspond to either very low mean accuracy ($\bar{q}=0.12$, noise floor) or very high mean accuracy ($\bar{q}=0.81$, ceiling-effect compression of logit variance). The discrepancy across biological datasets reflects statistical power: K=118 with 50 balanced trials per image provides sufficient power to detect the class-level correlation; $K\leq11$ does not.

Exploratory per-area analysis on one session (session ses-721123822) separates units by recorded brain region: combined visual cortex (r=0.869, n=579 active units) $>$ anteromedial (VISam: r=0.802) $>$ lateral (VISl: r=0.774) $>$ primary V1 (VISp: r=0.707) $>$ anterolateral (VISal: r=0.658) $>$ LP thalamus (r=0.519) $\geq$ rostrolateral (VISrl: r=0.445). The law holds across all visual processing stages; higher-order areas show stronger compliance, consistent with the visual hierarchy.

Two-level findings: (1)~\textbf{Form}: geometric margin predicts accuracy in all datasets---$r=0.41$--$0.75$ per-image (macaque IT, mouse V1, mouse VC), 30/32 Allen sessions pass H1 and all 32 show positive $r$, all mouse visual areas show positive $r$---confirming the EVT-derived law shape is substrate-independent. (2)~\textbf{Constant}: $A_{\mathrm{renorm,bio}}\in[0.03, 0.07]$ versus $A_{\mathrm{renorm,NLP}}=1.13$---biological visual codes are $\approx15$--$34\times$ less geometrically efficient per unit of $\kappa_{\mathrm{nearest}}$ than trained artificial networks, consistent with the interpretation that gradient-based training optimises the constant while leaving the geometric skeleton intact.
...
\item \textbf{Biological neurons.} Three biological datasets confirm the law form (see Discussion: Cross-species validation) but reject substrate-independent constants ($A_{\mathrm{bio}}\approx15$--$34\times$ smaller than $A_\mathrm{NLP}$). All three pre-registered H4 tests (substrate independence requiring H1 $\land$ H2) fail. Class-level $H_1$ passes in 30/32 Allen sessions (all 32 show positive $r$, significant at $p<0.001$); $K=7$ (macaque IT) and $K=11$ (mouse V1) are underpowered. The two non-passing Allen sessions have below-threshold $r$ due to noise-floor ($\bar{q}=0.12$) or ceiling-effect ($\bar{q}=0.81$) regimes rather than law failure. A genuine substrate-independence test requires either (a) many biological recording sessions treated as ``architectures'' in a LOAO design, or (b) a theory predicting how $A$ scales with training algorithm and neural code efficiency.
...
\item \textbf{Two-component geometry structure and causal transferability.} Two pre-registered decoupling experiments probe the relationship between $\kappa_{\mathrm{nearest}}$ (total SNR: $\delta_{\min}/\sigma_W\sqrt{d}$) and $\kappa_{\mathrm{eff}}$ (directional SNR: $\delta_{\min}/\sigma_{\mathrm{cd}}$). \textit{Null-space identifiability} (commit \texttt{6938940}; \texttt{cti\_kappa\_eff\_identifiability.json}): null-space scaling provably holds $\kappa_{\mathrm{eff}}$ invariant while changing $\kappa_{\mathrm{nearest}}$ ($<0.001\%$ variation, H1: 3/3); $q$ tracks $\kappa_{\mathrm{nearest}}$ (OLMo-1B: $r=1.00$), not $\kappa_{\mathrm{eff}}$ (H4: 1/3 PASS). \textit{2D causal surface} (commit \texttt{2bc7da6}; \texttt{cti\_2d\_causal\_surface.json}): a $3\times3$ factorial orthogonally manipulates both components on 2 NLP architectures. A bivariate model $\logit(q)=\alpha_1\kappa_{\mathrm{nearest}}+\alpha_2\kappa_{\mathrm{eff}}+C$ does not significantly outperform the univariate $\kappa_{\mathrm{nearest}}$ law (H4 FAIL: $\Delta R^2=0.03/0.001$; $R^2=0.92$--$0.98$ for $\kappa_{\mathrm{nearest}}$ alone). \textbf{Cross-architecture bivariate transfer is strong} (H6: $r=0.988$, $0.967$), confirming law robustness across architecture families. A dimensional confound limits the factorial: in $d=2048$ with $\approx$2029 null dimensions, compensated signal-scaling redistributes high-dimensional null noise in ways not captured by the one-parameter formula. Combined, these experiments establish $\kappa_{\mathrm{nearest}}$ as the dominant causal predictor; $d_{\mathrm{eff}}$ adds marginal information, and the causal law transfers across architectures ($r>0.96$). A renormalized extension accounting for null-space dimensionality is a future direction.
\end{enumerate}
