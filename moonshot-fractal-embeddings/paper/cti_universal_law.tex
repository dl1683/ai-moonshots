...
\begin{table}[h]
\centering
\caption{N\textsubscript{eff} geometric exponent and competition density $\rho$ across four architectures
on DBpedia ($K=14$). Pearson $r=0.996$, Spearman $\rho=1.000$ between exponent and density.}
\label{tab:neff-competition-density}
\small
\begin{tabular}{lcccc}
\toprule
Architecture & Exponent $\beta_\mathrm{geom}$ & $N_\mathrm{eff}$ at $K=14$ & $\rho = N_\mathrm{eff}/(K-1)$ & $r$ \\
\midrule
Pythia-160m  & 0.934 & 10.45 & 0.804 & 0.9999 \\
OLMo-1B      & 0.806 & 7.51  & 0.578 & 0.9995 \\
Qwen3-0.6B   & 0.791 & 7.35  & 0.566 & 0.9998 \\
GPT-Neo-125M & 0.711 & 5.91  & 0.455 & 0.9995 \\
\midrule
\textbf{Corr.} & \multicolumn{4}{c}{Pearson $r(\rho, \beta_\mathrm{geom})=0.996$ ($p=0.004$);
  Spearman $\rho=1.000$} \\
\bottomrule
\end{tabular}
\end{table}
...
Across four architectures (Pythia-160m, GPT-Neo-125M, OLMo-1B, Qwen3-0.6B), the geometric
$N_\mathrm{eff}$ exponent ranges from $0.71$ to $0.93$ (mean $0.81$, CV$=9.8\%$; see
\texttt{results/cti\_neff\_pairwise\_multiarch.json}).
The exponent correlates perfectly with competition density $\rho=N_\mathrm{eff}(K_\mathrm{full})/(K_\mathrm{full}-1)$
at the full class count: Pearson $r=0.996$ ($p=0.004$), Spearman $\rho=1.000$.
Architectures that create denser, more uniform inter-class separation
(pythia-160m: $\rho=0.80$) exhibit steeper geometric scaling; those with sparser competition
