...
\item \textbf{K-scaling and practical prediction}: The slope $A(K)=1.164/\log(K)+1.301$ ($r_\mathrm{fit}=0.93$) predicts held-out dataset slopes within 50\%. One-shot dataset calibration achieves MAE$\leq$0.08 on all tested single-label datasets. Prospective validation on Banking77 ($K=77$, pre-registered, never seen in fitting): rho$=0.890$ \textbf{PASS}.
\end{enumerate}
...
\end{itemize}
Banking77 ($K=77$) is a strong result: rho$=0.890\geq0.85$ across all 6 anchors with frozen pre-registered $A(K=77)$. This is the first genuinely prospective pass (pre-registered formula, unseen dataset). The K=6 rho failure and K=77 rho pass together suggest the ranking-universality threshold requires $K\geq10$--$14$ for sufficient between-architecture discriminability.
