...
\textbf{Honest scope.} The constant $\alpha$ varies substantially by modality (NLP: 1.48, ViT: 0.63, CNN: 4.4); universality is of the \emph{functional form}, not of the constant. At $K=100$, $r\approx0.75$ falls below the Monte Carlo noise floor ($E[r]=0.875$), attributed to CIFAR-100's hierarchical class structure. The slope scales across datasets as $A(K)=1.164/\log(K)+1.301$ ($r_\mathrm{fit}=0.93$); one-shot dataset calibration achieves MAE$\leq$0.08 on all four prospective datasets (pre-registered, $K\in\{6,10,59,77\}$). Architecture-ranking ($\rho\geq0.85$) requires $K\geq59$. No external replication exists yet.
\end{abstract}
...
\item \textbf{K-scaling and practical prediction}: The slope $A(K)=1.164/\log(K)+1.301$ ($r_\mathrm{fit}=0.93$) predicts held-out dataset slopes within 50\%. One-shot dataset calibration achieves MAE$\leq$0.08 on \emph{all} four prospective datasets (pre-registered, never seen in fitting). Architecture-ranking ($\rho\geq0.85$) requires $K\geq59$; K-sweep spanning $K\in\{6,10,59,77\}$ establishes the threshold in the range $10$--$59$.
\end{enumerate}
...
To locate the $K$-threshold for reliable ranking, we pre-register a second prospective test (\texttt{src/cti\_prospective\_k\_sweep.py}, commit \texttt{88b3699}) on two intermediate-$K$ datasets:
\begin{itemize}[nosep]
\item \textbf{Yahoo Answers Topics} (K=10, $A_\mathrm{pred}=1.807$, 6 architectures): MAE$=0.049$ \textbf{PASS H2}; $\rho=0.835$ FAIL H1.
\item \textbf{Amazon Massive Intent} (SetFit/amazon-massive-intent-en-US, $K=59$, $A_\mathrm{pred}=1.587$, 6 architectures): MAE$=0.055$ \textbf{PASS H2}; $\rho=0.922$ \textbf{PASS H1}.
\end{itemize}
The combined K-sweep across all four prospective datasets establishes a clear empirical pattern:
\begin{center}
\begin{tabular}{lrrr}
\toprule
Dataset & $K$ & $\rho$ & H1 ($\rho\geq0.85$) \\
\midrule
emotion        & 6  & 0.613 & FAIL \\
Yahoo Topics   & 10 & 0.835 & FAIL \\
Amazon Massive & 59 & 0.922 & PASS \\
Banking77      & 77 & 0.890 & PASS \\
\bottomrule
\end{tabular}
\end{center}
MAE universality (H2: MAE$\leq$0.08) holds for all four prospective datasets, confirming practical magnitude prediction with a single-anchor calibration. Architecture-ranking reliability ($\rho\geq0.85$) consistently requires $K\geq59$; at $K=10$ the threshold is nearly met ($\rho=0.835$), establishing the K-threshold in the range $10$--$59$.
