...
For mouse visual cortex (Allen Neuropixels, DANDI:000021, $K=118$ natural images, $n=5{,}900$, 50 trials/image): we ran the identical protocol on 7 independent recording sessions from 7 different mice. All 7 sessions satisfy H1: $r(\kappa_{\mathrm{nearest}}, \logit q)\in[0.51, 0.89]$, mean$=0.73$, inter-session CV$=17.8\%$; per-image $r\in[0.48, 0.77]$, mean$=0.69$ (\texttt{results/cti\_allen\_all\_sessions.json}). The discrepancy across biological datasets reflects statistical power: K=118 with 50 balanced trials per image provides sufficient power to detect the class-level correlation; $K\leq11$ does not.
...
Two-level findings: (1)~\textbf{Form}: geometric margin predicts accuracy in all datasets---$r=0.41$--$0.75$ per-image (macaque IT, mouse V1, mouse VC), 7/7 Allen sessions pass H1, all mouse visual areas show positive $r$---confirming the EVT-derived law shape is substrate-independent. (2)~\textbf{Constant}: $A_{\mathrm{renorm,bio}}\in[0.03, 0.07]$ versus $A_{\mathrm{renorm,NLP}}=1.13$---biological visual codes are $\approx15$--$34\times$ less geometrically efficient per unit of $\kappa_{\mathrm{nearest}}$ than trained artificial networks, consistent with the interpretation that gradient-based training optimises the constant while leaving the geometric skeleton intact.
...
\item \textbf{Biological neurons.} Three biological datasets confirm the law form (see Discussion: Cross-species validation) but reject substrate-independent constants ($A_{\mathrm{bio}}\approx15$--$34\times$ smaller than $A_\mathrm{NLP}$). All three pre-registered H4 tests (substrate independence requiring H1 $\land$ H2) fail. Class-level $H_1$ passes only at $K=118$ (Allen Neuropixels); $K=7$ (macaque IT) and $K=11$ (mouse V1) are underpowered. A genuine substrate-independence test requires either (a) many biological recording sessions treated as ``architectures'' in a LOAO design, or (b) a theory predicting how $A$ scales with training algorithm and neural code efficiency.
