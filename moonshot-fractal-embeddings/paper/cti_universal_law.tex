...
For mouse visual cortex (Allen Neuropixels, DANDI:000021, $K=118$ natural images, 963 units, $n=5{,}900$, 50 trials/image): class-level $r(\kappa_{\mathrm{nearest}}, \logit q)=0.851$ ($p=3\times10^{-34}$; pre-registered threshold $r>0.50$: \textbf{PASS}); per-image $r=0.747$ ($p<10^{-4}$). The discrepancy across biological datasets reflects statistical power: K=118 with 50 balanced trials per image provides sufficient power to detect the class-level correlation; $K\leq11$ does not.

Two-level findings: (1)~\textbf{Form}: margin predicts accuracy in all three datasets ($r=0.41$--$0.75$ per-image), confirming the EVT-derived law shape is substrate-independent. (2)~\textbf{Constant}: $A_{\mathrm{renorm,bio}}\in[0.03, 0.07]$ versus $A_{\mathrm{renorm,NLP}}=1.13$---biological visual codes are $\approx15$--$34\times$ less geometrically efficient per unit of $\kappa_{\mathrm{nearest}}$ than trained artificial networks, consistent with the interpretation that training optimises the pre-constant while leaving the geometric skeleton intact.
