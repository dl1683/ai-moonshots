...
Frozen do-interventions and an orthogonal factorial identify nearest-class geometry as the sole causal driver.
...
Biological generalization: 30/32 mouse visual-cortex Neuropixels sessions satisfy the law ($\bar{r}_\kappa=0.736$, pre-registered).
...
The empirical $\alpha\approx1.477$ is post-hoc consistent with $\alpha_\mathrm{theory}=\sqrt{4/\pi}\cdot\sqrt{d_\mathrm{eff}}$ (back-calculated $d_\mathrm{eff}\approx1.71$), but direct geometric measurement gives $d_\mathrm{eff}=9$--$151$ across architectures and $A_\mathrm{renorm}=\alpha/\sqrt{d_\mathrm{eff}}$ has $\mathrm{CV}=0.82$ (not universal); this is internal consistency, not independent corroboration.
...
Causal confusion-matrix prediction at all three shift deltas ($\delta\in\{1,2,3\}$) passes
H1 and H3 with no refit: $r=0.842$--$0.776$, sign accuracy $92.9$--$100\%$.
See Appendix~\ref{app:competition-field} for the full 11-experiment summary table and details.
...
The encoder family shows large within-family $\alpha$ variation ($\mathrm{CV}=0.92$), consistent with pooling protocol and pre-training objective jointly determining encoder $\alpha$ (pre-registered LOAO, commit \texttt{9e151d5}).
