...
Leave-one-architecture-out (LOAO) over 12 architectures yields mean $\alpha=2.868$, std $0.055$, coefficient of variation $\mathbf{0.019}$ --- 13$\times$ better than the pre-registered threshold of 0.25. Architectures span transformers (Pythia, GPT-Neo, OLMo, TinyLlama, Qwen, Mistral), a Transformer+Mamba hybrid (Falcon-H1), and a pure linear RNN with zero attention (RWKV-4). RWKV's held-out $\alpha=2.887$ falls within the pre-registered acceptance interval $[2.43, 3.29]$, a pre-planned boundary test. Cross-modality evaluation on ViT-Large yields $R^2=0.964$, supporting structural universality with a modality-specific slope ($A_{\mathrm{ViT}}=0.630$ vs. $A_{\mathrm{NLP}}=1.054$). Causal evidence from frozen do-interventions and an orthogonal factorial test confirms nearest-class geometry as the dominant causal driver.
\end{abstract}
...
\item \textbf{Cross-modal validation}: ViT-Large reaches $R^2=0.964$ with same functional form.
...
\subsection{Cross-modal ViT results}
\begin{table}[t]
\centering
\caption{Cross-modal ViT results. $A_\mathrm{fit}$ is the slope constant in $\logit(q)=A\cdot\kappa\sqrt{d_\mathrm{eff}}+C$.}
\label{tab:vit}
\small
\begin{tabular}{lcccc}
\toprule
Model & Layers & $A_{\mathrm{fit}}$ & $R^2$ & Pearson $r$ \\
\midrule
ViT-Base-16-224 & 12 & 0.592 & 0.811 & 0.901 \\
ViT-Large-16-224 & 24 & 0.630 & 0.964 & 0.982 \\
\midrule
NLP reference & -- & 1.054 & -- & -- \\
\bottomrule
\end{tabular}
...
ViT-Large reaches $R^2=0.964$, matching the NLP law fit in terms of explained variance. The slope constant $A_{\mathrm{ViT}}\approx0.63$ differs from $A_{\mathrm{NLP}}=1.054$ by a factor of 1.67$\times$, consistent with modality-specific geometry (vision representations are less anisotropic in the nearest-class direction than text representations). This supports \emph{structural universality} of the functional form with \emph{modality-specific} constants.
...
Three claims are strongly supported:
...
\item \textbf{Vision scope.} Cross-modal evidence is on CIFAR-10 ViT settings only; other vision architectures (ResNet, ConvNeXt) remain untested.
...
\logit(q_{\mathrm{norm}})=\alpha\kappa_{\mathrm{nearest}}-\beta\log(K-1)+C_0,
\]
derived from extreme-value theory and validated across 12 architectures (LOAO $\mathrm{CV}_\alpha=0.019$). The law spans transformers, SSM hybrids, and pure linear RNNs (RWKV), with pre-registered boundary tests providing strong evidence of genuine universality. Cross-modal ViT results confirm the same functional form with modality-specific constants. The next step is a renormalization theory for how constants transform across tasks and modalities.

\section*{Reproducibility Statement}
All numerical claims are taken directly from repository artifacts: \texttt{results/cti\_kappa\_nearest\_universal.json}, \texttt{results/rwkv\_preregistration.json}, \texttt{results/cti\_kappa\_loao\_per\_dataset.json}, \texttt{results/cti\_vit\_cross\_modality.json}, \texttt{results/cti\_do\_intervention\_text.json}, \texttt{results/cti\_orthogonal\_factorial.json}. Pre-registration timestamps are verifiable via git log. Code is available at [repository URL].
