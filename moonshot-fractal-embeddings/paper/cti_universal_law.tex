...
Two-level findings: (1)~\textbf{Form}: geometric margin predicts accuracy in all datasets---$r=0.41$--$0.75$ per-image (macaque IT, mouse V1, mouse VC), 7/7 Allen sessions pass H1---confirming the EVT-derived law shape is substrate-independent. (2)~\textbf{Constant}: $A_{\mathrm{renorm,bio}}\in[0.03, 0.07]$ versus $A_{\mathrm{renorm,NLP}}=1.13$---biological visual codes are $\approx15$--$34\times$ less geometrically efficient per unit of $\kappa_{\mathrm{nearest}}$ than trained artificial networks, consistent with the interpretation that gradient-based training optimises the constant while leaving the geometric skeleton intact.
