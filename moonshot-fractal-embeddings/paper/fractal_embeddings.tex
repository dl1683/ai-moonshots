...
\begin{figure}[t]
    ...
    \caption{Cross-dataset steerability: V5 (blue) produces positive steerability that scales with hierarchy depth, while MRL (orange) remains near zero across all datasets. Error bars show 95\% CIs.}
    \label{fig:forest}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{Causal ablation on CLINC-150 (5 seeds). Aligned supervision (V5) produces positive steerability; inverting alignment reverses the sign; removing prefix-specific alignment collapses it. Individual seed values shown as dots.}
    \label{fig:ablation}
\end{figure}
...
\paragraph{Results.}
We evaluate on 500 cross-$L_0$ pairs from CLINC-150.
If V5 truly separates semantic levels, we expect the chimera's $L_0$ prediction to follow the donor (high coarse transfer rate) while its $L_1$ prediction follows the recipient (high fine preservation rate).
MRL chimeras, lacking semantic separation, should show lower rates on both metrics.
This ``prefix surgery'' is the strongest form of causal test: it manipulates the \emph{representation}, not the training signal.
...
\begin{figure}[t]
    ...
    \caption{Steerability scales with hierarchy refinement entropy $\hlo$ across real datasets (Spearman $\rho = 1.0$, $p = 0.042$).}
    \label{fig:scaling}
\end{figure}
...
TREC ($\hlo = 2.21$) & $+0.045$ & $+0.081 \pm 0.012$ & $+0.003$ & $+0.011 \pm 0.002$ \\
\bottomrule
\end{tabular}
\end{table}
