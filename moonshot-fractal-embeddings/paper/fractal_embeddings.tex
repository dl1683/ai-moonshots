...
Specifically, $4.4\times$ more trainable parameters cannot recover steerability without hierarchy-aligned prefix supervision ($p = 0.40$ and $p = 0.71$ for MRL-frozen vs.\ MRL+backbone on CLINC and DBPedia respectively).
Adding backbone fine-tuning to V5 also yields no improvement ($p = 0.57$ on CLINC), confirming that alignment is both necessary and sufficient under these conditions.
