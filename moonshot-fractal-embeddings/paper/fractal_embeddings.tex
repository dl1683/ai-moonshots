...
Concurrent work on coarse-to-fine retrieval~\citep{jiang2025funnelrag} and multi-granularity RAG interfaces~\citep{chen2026arag} address the granularity problem through external pipeline mechanisms (multiple retrievers, separate tools) rather than encoding granularity into the vector itself.
Our work is the first to embed multi-level semantic granularity in a single vector's prefix structure.
...
A factorial backbone control confirms alignment is both necessary and sufficient: $4.4\times$ more trainable parameters without alignment produce zero steerability ($d = 12.9$).
