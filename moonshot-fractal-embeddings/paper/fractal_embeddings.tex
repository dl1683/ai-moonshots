...
\paragraph{Datasets.}
Table~\ref{tab:datasets} summarises eight evaluation datasets spanning conditional entropies from 1.23 (Yahoo~\citep{zhang2015yahoo}) to 5.05 bits (WOS~\citep{kowsari2017hdltex}), including GoEmotions~\citep{demszky2020goemotions}, TREC~\citep{voorhees2000trec}, 20 Newsgroups~\citep{lang1995newsgroups}, arXiv~\citep{clement2019arxiv}, DBPedia Classes~\citep{lehmann2015dbpedia}, and CLINC-150~\citep{larson2019clinc}.
...
\begin{table}[t]
\caption{Million-scale retrieval on BEIR DBPedia-Entity (4.6M docs). nDCG@10 at each prefix dimension. SVD rotation improves over naive truncation; hierarchy alignment incurs ${\leq}3\%$ cost.}
\label{tab:beir}
\centering
\small
\begin{tabular}{lcccc}
\toprule
Method & 64d & 128d & 192d & 256d \\
\midrule
Naive truncation & 0.182 & 0.301 & 0.336 & 0.356 \\
SVD rotation & \textbf{0.252} & \textbf{0.317} & \textbf{0.348} & \textbf{0.365} \\
Hierarchy-aligned ($\alpha = 0.01$) & 0.237 & 0.309 & 0.339 & 0.353 \\
Classification-only (V5 proj.) & 0.021 & 0.033 & 0.039 & 0.042 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Natural deep hierarchies.}
To test generalisation beyond curated text classification datasets, we evaluate on the Harvard USPTO Patent Dataset (HUPD)~\citep{suzgun2024hupd}, which provides a natural 2-level hierarchy: 8 patent sections $\rightarrow$ 121 technology classes ($\hlo = 2.42$ bits).
On 5 seeds, V5 achieves $\steer = +0.043 \pm 0.017$ versus MRL's $-0.002 \pm 0.016$ (paired $t = 3.63$, $p = 0.022$, $d = 1.8$).
This confirms that V5's steerability advantage holds on a real-world taxonomy with 121 fine-grained classes, not just curated benchmarks.
...
\begin{itemize}
    ...
    \item \textbf{Deeper hierarchies.} HUPD Section$\rightarrow$Class ($K_1 = 121$) confirms generalisation with $d = 1.8$. The 3-level CLINC extension yields $d = 10.9$ but only 5 super-domain classes leave $L_0$ near ceiling. Evaluation on even deeper taxonomies (Section$\rightarrow$Subclass with $K_1 = 587$) is in progress.
    ...
    \item \textbf{Baseline scope.} We compare against a matched MRL baseline controlling all variables except the prefix-to-label mapping. We do not benchmark HEAL~\citep{zhang2025heal}, CSR~\citep{chen2025csr}, or SMEC~\citep{li2025smrl} because these address orthogonal goals---none offers prefix-level steering, making fixed-dimensionality accuracy comparisons inapposite.
...
More broadly, the recent single-letter characterization of mismatched distortion-rate functions~\citep{letreust2025mismatched} establishes that encoder-decoder objective misalignment (flat training evaluated by hierarchical steerability) incurs a strictly positive penalty whenever $H(L_1|L_0) > 0$.
