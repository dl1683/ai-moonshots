...
Across nine datasets spanning conditional entropies from 1.23 to 5.05 bits, a random-effects meta-analysis yields pooled $d = 1.48$ ($p < 0.0001$), with all nine favouring fractal training (sign test $p = 0.002$).
On CLINC-150, steerability reaches $\steer = +0.150 \pm 0.028$ versus MRL's $+0.007 \pm 0.016$ ($p_\text{adj} = 0.005$, $d = 4.3$).
A synthetic hierarchy experiment identifies a Goldilocks optimum where steerability peaks at capacity--demand matching (quadratic $R^2 = 0.964$), and steerability magnitude across real datasets is predicted by the product of hierarchy depth and baseline learnability ($\rho = 0.85$, $p = 0.004$).
A backbone fine-tuning control on three datasets confirms that alignment is both necessary and sufficient: a flat MRL baseline with $4.5\times$ more trainable parameters (backbone fine-tuning) produces near-zero steerability ($d = 11.6$, $7.4$, and $4.4$ vs.\ head-only V5 on CLINC, DBPedia Classes, and TREC respectively).
Replication across three encoder families confirms architecture invariance.
Beyond classification, prefix specialisation transfers to million-scale retrieval (BEIR DBPedia-Entity, 4.6M documents): a hierarchy-aligned projection achieves 94--97\% of SVD-optimal retrieval quality while maintaining semantic steering.
Evaluation on a natural deep hierarchy (HUPD patent taxonomy, 121 classes) confirms V5's advantage on real taxonomies ($d = 1.8$, $p = 0.022$).
We ground these findings in the classical theory of successive refinement, proving that steerability equals the refinement mutual information $I(M_2; L_1|L_0, M_1)$ and deriving a converse bound showing flat supervision \emph{cannot} achieve high steerability (the V5 advantage grows linearly with excess prefix capacity), alongside a Goldilocks capacity--demand optimum and product scaling law.
\end{abstract}
...
Our evaluation spans nine hierarchical text datasets, three encoder families, four controlled ablations, a synthetic causal experiment, and connections to 30 years of information theory.
...
\begin{enumerate}
    ...
    \item \textbf{A scaling analysis} linking steerability magnitude to the interaction of hierarchy depth and model learnability across nine datasets ($\rho = 0.85$, $p = 0.004$), with a Goldilocks optimum identified via synthetic intervention ($R^2 = 0.964$; Section~\ref{sec:scaling}).
    \item \textbf{Broad generality}: cross-model replication on BGE-small, E5-small, and Qwen3-0.6B; downstream retrieval benchmarks showing $10\times$ larger dimensionality--accuracy ramps; transfer to million-scale retrieval (BEIR DBPedia-Entity, 4.6M documents); validation on natural deep hierarchies (HUPD patent taxonomy); and $3.7\times$ HNSW query speedups from prefix routing (Section~\ref{sec:generality}).
    \item \textbf{Information-theoretic foundations}: a steerability identity ($\steer = I(M_2; L_1|L_0, M_1)$), a converse bound proving flat supervision \emph{cannot} achieve high steerability, and a constructive achievability result showing V5's advantage grows linearly with excess prefix capacity (Section~\ref{sec:theory}).
\end{enumerate}
...
Paired $t$-tests compare V5 and MRL steerability per dataset; the nine main comparisons (Table~\ref{tab:steerability}) are corrected via Holm--Bonferroni~\citep{holm1979simple} at $\alpha = 0.05$.
...
At full embedding length ($j=4$, 256d), V5 and MRL achieve comparable $k$-NN accuracy across all nine datasets (Table~\ref{tab:accuracy}).
...
\begin{figure}[t]
    ...
    \caption{Steerability across nine datasets. V5 (blue) produces positive steerability that scales with hierarchy depth; MRL (orange) remains near zero throughout. Error bars: 95\% CIs across 5 seeds.}
    \label{fig:forest}
\end{figure}

\paragraph{Steerability emerges from hierarchy alignment.}
Despite comparable full-resolution accuracy, V5 produces dramatically higher steerability than MRL on all nine datasets (Figure~\ref{fig:forest}, Table~\ref{tab:steerability}).
...
After Holm--Bonferroni correction ($m = 9$), three datasets reach significance: DBPedia Classes ($p_\text{adj} = 0.002$, $d = 5.5$), CLINC ($p_\text{adj} = 0.005$, $d = 4.3$), and TREC ($p_\text{adj} = 0.044$, $d = 2.4$).
ArXiv is borderline ($p_\text{adj} = 0.093$, $d = 1.8$).
...
A sign test confirms universal directionality: V5 $>$ MRL on 9/9 datasets ($p = 0.002$, binomial).
A DerSimonian--Laird meta-analysis yields pooled $d = 1.48$ (95\% CI: $[0.76, 2.19]$, $z = 4.04$, $p < 0.0001$).
The 95\% prediction interval for a new dataset is $[-0.60, 3.56]$, reflecting moderate heterogeneity ($I^2 = 59\%$) that the scaling trend analysis explains as systematic moderation by hierarchy depth and learnability.

\begin{table}[t]
\caption{Steerability across nine datasets (Eq.~\ref{eq:steerability}). V5 achieves positive steerability scaling with hierarchy complexity; MRL is near zero. Mean $\pm$ SD over 5 seeds. The first eight are standard text benchmarks; HUPD is a natural patent taxonomy.}
...
\begin{figure}[t]
    ...
    \caption{\textbf{Left:} Steerability vs.\ $\hlo$ across nine datasets ($\rho = 0.67$, $p = 0.049$). WOS deviates due to a floor effect. \textbf{Right:} Product predictor $\hlo \times A_{L_1}^\text{base}$ accounts for both complexity and learnability ($\rho = 0.85$, $p = 0.004$).}
    \label{fig:scaling}
\end{figure}

Across nine datasets, steerability increases with hierarchy depth: Spearman $\rho = 0.67$ ($p = 0.049$) against $\hlo$ alone (Figure~\ref{fig:scaling}, left).
...
\begin{itemize}
    \item $\hlo$ alone: $\rho = 0.67$ ($p = 0.049$)
    \item $A_{L_1}^\text{base}$ alone: $\rho = 0.67$ ($p = 0.049$)
    \item $\hlo \times A_{L_1}^\text{base}$: $\rho = 0.85$ ($p = 0.004$); Pearson $r = 0.97$ ($p < 0.001$)
\end{itemize}
...
A leave-one-out analysis confirms robustness: all LOO $\rho \geq 0.53$, and bootstrap 95\% CI for $\rho(\hlo)$ is $[-0.06, 1.0]$ with 96.7\% positive (Appendix~\ref{app:scaling_robust}).
...
The Harvard USPTO Patent Dataset (HUPD)~\citep{suzgun2024hupd} provides a natural 2-level hierarchy: 8 patent sections $\rightarrow$ 121 technology classes ($\hlo = 2.42$ bits).
Unlike our other eight datasets (curated text classification benchmarks), HUPD's hierarchy emerges from real patent office taxonomic practice, providing an important external validity check.
V5 achieves $\steer = +0.043 \pm 0.019$ versus MRL's $-0.002 \pm 0.017$ ($d = 1.8$, $p_\text{adj} = 0.110$; Table~\ref{tab:steerability}), confirming that V5's steerability advantage holds on real-world taxonomies.
...
We compare against a matched MRL baseline controlling all variables except the prefix-to-label mapping.
We do not benchmark HEAL~\citep{zhang2025heal}, CSR~\citep{chen2025csr}, or SMEC~\citep{li2025smrl} because these address orthogonal goals---none offers prefix-level steering, making fixed-dimensionality accuracy comparisons inapposite.
...
\emph{V5 achievability.}
Under hierarchy-aligned supervision with $I(M_1; L_1|L_0) = 0$ and $R_1 \geq H(L_0)$, we have $H(L_1|L_0, M_1) = H(L_1|L_0)$.
Stage~2 encodes $L_1|L_0$ at rate $R_2$; when $R_2 \geq H(L_1|L_0)$, achievability by standard source coding gives $\steer_\text{V5} = H(L_1|L_0)$.
...
The product $\hlo \cdot A_{L_1}^\text{base}$ thus captures both the amount of refinement information available and the model's capacity to exploit it, explaining the empirical $\rho = 0.85$ across nine datasets.
...
\emph{Empirically}, fractal training produces steerability on all nine datasets (pooled $d = 1.48$, $p < 0.0001$), with magnitude predicted by the product of hierarchy depth and baseline learnability ($\rho = 0.85$).
...
\begin{table}[h]
\caption{Metric robustness: V5$\,$--$\,$MRL gap under four steerability formulations. Three of four agree V5 $>$ MRL on all nine datasets.}
...
\midrule
V5 $>$ MRL & & 9/9 & 9/9 & 7/9 & 9/9 \\
Sign test $p$ & & 0.002 & 0.002 & 0.09 & 0.002 \\
\bottomrule
\end{tabular}
\end{table}

All pairwise rank correlations between the original, AUC, and gap metrics exceed $\rho = 0.98$ ($p < 0.001$), confirming these formulations recover the same dataset ordering. The monotonicity metric shows weaker agreement ($\rho = 0.68$, $p = 0.045$) but still supports V5 on 7/9 datasets.

\section{Per-Seed Steerability Values}
\label{app:perseed}

\begin{table}[h]
\caption{Per-seed steerability (V5 and MRL) across all nine datasets. Seeds: 42, 123, 456, 789, 1024.}
\label{tab:perseed}
\centering
\scriptsize
...
\begin{table}[h]
\caption{Leave-one-out sensitivity for the $\hlo$ scaling trend. WOS (Cook's $D = 4.16$) is the highest-influence point; the product predictor resolves its deviation without dropping any dataset.}
\label{tab:loo}
\centering
\small
\begin{tabular}{lccccc}
...
\midrule
None (full) & 9 & 0.67 & 0.049 & 0.49 & 0.176 \\
\midrule
Yahoo & 8 & 0.53 & 0.180 & 0.41 & 0.310 \\
GoEmotions & 8 & 0.55 & 0.160 & 0.45 & 0.258 \\
Newsgroups & 8 & 0.62 & 0.102 & 0.48 & 0.233 \\
TREC & 8 & 0.74 & 0.035 & 0.49 & 0.219 \\
arXiv & 8 & 0.73 & 0.040 & 0.50 & 0.206 \\
HUPD sec$\rightarrow$cls & 8 & 0.74 & 0.035 & 0.49 & 0.218 \\
DBPedia Classes & 8 & 0.62 & 0.099 & 0.49 & 0.214 \\
CLINC & 8 & 0.62 & 0.099 & 0.34 & 0.412 \\
WOS & 8 & 0.81 & 0.014 & 0.91 & 0.002 \\
\bottomrule
\end{tabular}
\end{table}

Bootstrap analysis (10{,}000 resamples): 96.7\% of $\rho(\hlo)$ values positive.
Meta-analysis prediction interval $[-0.60, 3.56]$ reflects heterogeneity ($I^2 = 59\%$) explained by the interaction model.
