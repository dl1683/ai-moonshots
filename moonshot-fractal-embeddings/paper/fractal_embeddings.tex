...
HEAL achieves substantially higher steerability than V5 on every dataset (mean ratio ${\sim}4\times$; overall paired $t = 10.2$, $p < 10^{-6}$).
...
Among hierarchy-aware approaches, V5's contribution is not highest raw steerability but rather: (i)~the simplest mechanism (single linear projection, ${\sim}$130K head parameters trained for 5 epochs with cross-entropy, vs.\ HEAL's ${\sim}$600K-parameter trunk with level-specific projectors trained for 30 epochs with $O(n^2)$ contrastive loss), (ii)~the formal successive refinement framework predicting \emph{when} and \emph{how much} steerability emerges (Section~\ref{sec:theory}), and (iii)~the predictive scaling law (Section~\ref{sec:scaling}).

\begin{table}[t]
\caption{External baseline comparison (BGE-small). HEAL, V5, and MRL: mean $\pm$ SD over 5 seeds. CSR and SMEC: representative seed (steerability near zero across all tested datasets). HEAL (hierarchy-aware) achieves highest steerability via architectural separation; V5 achieves it through training alone; CSR and SMEC (hierarchy-unaware) produce no steerability.}
\label{tab:external}
\centering
\small
\begin{tabular}{lcccccc}
...
\midrule
Yahoo & 1.23 & $+0.115 \pm 0.003$ & $+0.015 \pm 0.019$ & $+0.005 \pm 0.011$ & --- & --- \\
GoEmotions & 1.88 & $+0.074 \pm 0.008$ & $+0.020 \pm 0.018$ & $+0.006 \pm 0.017$ & --- & --- \\
Newsgroups & 1.88 & $+0.103 \pm 0.008$ & $+0.035 \pm 0.029$ & $+0.000 \pm 0.016$ & --- & --- \\
TREC & 2.21 & $+0.203 \pm 0.014$ & $+0.044 \pm 0.017$ & $-0.001 \pm 0.015$ & $+0.006$ & --- \\
arXiv & 2.62 & $+0.132 \pm 0.009$ & $+0.027 \pm 0.015$ & $-0.001 \pm 0.013$ & --- & --- \\
DBPedia Cl. & 3.17 & $+0.351 \pm 0.017$ & $+0.120 \pm 0.016$ & $+0.008 \pm 0.008$ & --- & --- \\
CLINC & 3.90 & $+0.263$ & $+0.150 \pm 0.028$ & $+0.007 \pm 0.016$ & $+0.003$ & $-0.002$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\caption{Metric robustness: V5$\,$--$\,$MRL gap under four steerability formulations (original nine datasets). Three of four agree V5 $>$ MRL on all nine. All twelve datasets show 12/12 agreement under $\steer_\text{orig}$.}
...
All pairwise rank correlations between the original, AUC, and gap metrics exceed $\rho = 0.98$ ($p < 0.001$), confirming these formulations recover the same dataset ordering. The monotonicity metric shows weaker agreement ($\rho = 0.68$, $p = 0.045$) but still supports V5 on 7/9 of the original datasets.

\section{Per-Seed Steerability Values}
\label{app:perseed}

\begin{table}[h]
\caption{Per-seed steerability (V5 and MRL) across the original nine datasets. Seeds: 42, 123, 456, 789, 1024. Deep-hierarchy per-seed values are available in the supplementary materials.}
\label{tab:perseed}
\centering
\scriptsize
