...
BGE-small $+0.144$, E5-small $+0.130$, Qwen3 $+0.153$ (all $p < 0.005$).
...
\begin{table}[t]
\caption{Cross-model replication (3 seeds each). Steerability is architecture-invariant across three model families with different pre-training objectives and sizes. The V5 $\gg$ MRL pattern holds universally. All comparisons significant ($p < 0.005$).}
...
\midrule
BGE-small (BAAI, 33M) & $+0.144 \pm 0.037$ & $+0.015 \pm 0.011$ & $+0.045 \pm 0.019$ & $+0.003$ \\
...
We evaluate on CLINC-150 (deep hierarchy, $\hlo = 3.90$) and TREC (moderate, $\hlo = 2.21$), each with 3 seeds:

\begin{itemize}
    \item \textbf{V5 at 64d}: CLINC $L_0 = 97.5\%$, $L_1 = 81.3\%$; TREC $L_0 = 94.0\%$, $L_1 = 71.3\%$. In both cases the short prefix is coarse-specialized.
    \item \textbf{V5 at 256d}: CLINC $L_1 = 92.5\%$; TREC $L_1 = 74.5\%$. Full resolution recovers fine accuracy ($+11.2$pp on CLINC, $+3.2$pp on TREC).
    \item \textbf{MRL at 64d}: CLINC $L_1 = 91.2\%$; TREC $L_1 = 74.1\%$. MRL prefixes are \emph{not} coarse-specialized.
    \item \textbf{MRL at 256d}: CLINC $L_1 = 91.6\%$; TREC $L_1 = 73.9\%$. MRL shows negligible $L_1$ change ($+0.4$pp CLINC, $-0.2$pp TREC).
\end{itemize}
The same pattern holds across both hierarchy depths: V5 provides two distinct operating points---a fast 64d prefix optimized for coarse retrieval, and a full 256d embedding that recovers fine accuracy---enabling coarse-first, fine-on-demand retrieval pipelines.
MRL's flat $L_1$ profile across dimensions confirms it cannot support such adaptive strategies.
