...
Among hierarchy-aware approaches, V5's contribution is not highest raw steerability but rather: (i)~the simplest mechanism (loss modification only, no architectural changes), (ii)~the formal successive refinement framework predicting \emph{when} and \emph{how much} steerability emerges (Section~\ref{sec:theory}), and (iii)~the predictive scaling law (Section~\ref{sec:scaling}).
