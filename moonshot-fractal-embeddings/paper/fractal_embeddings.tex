...
Four controlled ablations establish that \emph{alignment}---not hierarchy awareness---causally drives steerability: inverting alignment reverses its sign; removing it or applying uniform multi-task training collapses it to zero (all $p \leq 0.03$, Cohen's $d \geq 2.7$).
...
A backbone fine-tuning control confirms that alignment is both necessary and sufficient: a flat MRL baseline with $4.4\times$ more trainable parameters (backbone fine-tuning) produces zero steerability ($d = 12.9$ vs.\ head-only V5).
...
Both methods improve over the unfinetuned baseline on most datasets.
The notable exception is CLINC ($K_1 = 150$): the 384d$\rightarrow$256d linear projection reduces $L_1$ $k$-NN accuracy (V5: 67.6\%, MRL: 70.4\%) relative to the 384d baseline (88.7\%), though both methods' classification heads achieve $>$95\% on the validation set.
This reflects V5's trade-off: the embedding space is reorganised for prefix-level specialization rather than maximising $k$-NN accuracy at full resolution.
...
All three patterns replicate across hierarchy depths ($\hlo = 2.21$ and $3.90$), with $d \geq 2.7$ throughout.
...
To validate the Goldilocks prediction on natural hierarchies, we sweep the scale dimension (prefix capacity) across $d_s \in \{16, 32, 48, 64, 96, 128\}$ on three datasets spanning the $\hlo$ range: Yahoo ($\hlo = 1.23$), TREC ($2.21$), and DBPedia Classes ($3.17$).
Three seeds per condition.
All datasets peak at $d_s = 16$ (the minimum tested), consistent with the synthetic finding that the default 64d prefix is already at or beyond the Goldilocks optimum for these hierarchies.
The key finding is in the \emph{scaling law at fixed capacity}: steerability at $d_s = 16$ scales nearly perfectly with conditional entropy ($\rho = 1.000$, $r = 0.999$, $p = 0.027$), confirming that hierarchy complexity drives the steerability signal at matched capacity.
...
We test this with a 4-arm experiment on CLINC and DBPedia Classes (5 seeds each):
(i) \textbf{V5-frozen} (hierarchy-aligned, head-only, 2.1M params);
(ii) \textbf{MRL-frozen} (flat MRL, head-only, 2.1M params);
(iii) \textbf{MRL+backbone} (flat MRL, head + last 4 backbone layers fine-tuned, 9.2M params);
(iv) \textbf{V5+backbone} (hierarchy-aligned, head + backbone, 9.2M params).
On CLINC: V5-frozen $\steer = +0.050 \pm 0.005$ vs.\ MRL+backbone $\steer = +0.003 \pm 0.002$ ($d = 12.9$, $p < 10^{-6}$).
Adding backbone capacity to MRL does not help ($\steer$ stays near zero); V5 backbone fine-tuning adds nothing over V5 head-only ($p = 0.57$).
Hierarchy alignment is both necessary and sufficient for steerability; capacity alone is neither.
...
Table~\ref{tab:crossmodel} confirms that the steerability gap is architecture-invariant: BGE-small $+0.144$, E5-small $+0.130$, Qwen3 $+0.153$ (all $p < 0.025$, Holm-corrected across 3 families).
...
Most closely related, \citet{hanley2025hierarchical} independently train multilingual Matryoshka embeddings where different dimension subsets capture story similarity at different granularity levels for news clustering.
Our work provides complementary contributions: a formal successive-refinement framework, an explicit steerability metric with causal ablations, evaluation across eight domains and three encoder families, and a scaling trend linking steerability to hierarchy structure.

\paragraph{Dimensional redundancy.}
\citet{dufter2025random} show that randomly removing 50\% of dimensions causes $<$10\% performance loss, revealing massive redundancy.
...
\citet{luan2025limits} prove that single-vector embedding expressiveness for top-$k$ retrieval is bounded by dimensionality, motivating multi-resolution access patterns.
...
\emph{Causally}, four controlled ablations on two datasets establish that the specific prefix-to-hierarchy alignment---not hierarchy awareness, not architecture, not optimiser---drives the effect; sign reversal under inversion and collapse under uniform multi-task training both replicate.
