...
Across seven real datasets spanning five domains, steerability is predicted by the product of hierarchy depth $\hlo$ and baseline learnability (Spearman $\rho = 0.86$, $p = 0.014$; Pearson $r = 0.97$), revealing an interaction: steerability requires \emph{both} hierarchical complexity and model capacity to resolve the fine task.
...
A random-effects meta-analysis (DerSimonian--Laird) across all seven datasets yields a pooled Cohen's $d = 1.24$ (95\% CI: $[0.54, 1.94]$, $z = 3.48$, $p = 0.0005$), confirming a robust overall V5 advantage.
The substantial heterogeneity ($I^2 = 53\%$) is expected: the scaling trend analysis (Section~\ref{sec:scaling}) predicts that effect size is moderated by the interaction of hierarchy depth and model learnability.
...
\begin{figure}[t]
    ...
    \caption{Steerability across seven real datasets. \textbf{Left:} Raw steerability vs.\ $\hlo$ ($\rho = 0.72$, $p = 0.068$). WOS (highest $\hlo$) falls below the trend due to a floor effect in $L_1$ accuracy. \textbf{Right:} Steerability vs.\ the product predictor $\hlo \times A_{L_1}^\text{base}$ ($\rho = 0.86$, $p = 0.014$), which accounts for both hierarchy complexity and baseline learnability.}
    \label{fig:scaling}
\end{figure}
...
Using all seven datasets, the raw Spearman correlation with $\hlo$ is $\rho = 0.72$ ($p = 0.068$); excluding WOS, the six remaining datasets yield $\rho = 0.78$ ($p = 0.066$).
...
\begin{itemize}
    \item $\hlo$ alone: Spearman $\rho = 0.72$ ($p = 0.068$)
    \item $A_{L_1}^\text{base}$ alone: Spearman $\rho = 0.54$ ($p = 0.22$)
    \item $\hlo \times A_{L_1}^\text{base}$: Spearman $\rho = 0.86$ ($p = 0.014$); Pearson $r = 0.97$ ($p < 0.001$)
\end{itemize}
...
When the pretrained model cannot already resolve fine-grained distinctions (low $A_{L_1}^\text{base}$), head-only training cannot create steerability, regardless of how deep the hierarchy is.

A leave-one-out sensitivity analysis (Appendix~\ref{app:scaling_robust}) confirms robustness: all LOO Spearman values remain positive ($\rho \geq 0.55$), and the bootstrap 95\% CI for the raw $\hlo$ correlation is $[-0.06, 1.0]$ with 96.4\% of resamples positive.
...
\begin{itemize}
    ...
    \item \textbf{Floor and ceiling effects}: On WOS ($K_1 = 336$), head-only training achieves only ${\sim}11$--$12\%$ $L_1$ accuracy (floor), limiting steerability despite high $\hlo = 5.05$. On DBPedia ($K_0 = 14$, $L_0 = 100\%$), there is no room for steerability improvement (ceiling). Steerability requires both hierarchical complexity and model capacity to learn the fine task.
...
Through controlled ablations on two datasets, synthetic hierarchy experiments, cross-model replication on three families, and a random-effects meta-analysis across seven datasets ($d = 1.24$, $p = 0.0005$), we support the \textbf{Fractal Embedding Principle}: steerability is driven by alignment, scales with the interaction of hierarchy depth and baseline learnability ($\rho = 0.86$), and is absent in standard MRL.
...
Table~\ref{tab:perseed} reports individual seed steerability values for all seven datasets, enabling full transparency about the variability underlying the summary statistics in Table~\ref{tab:steerability}.
...
With seven datasets, we provide a thorough sensitivity analysis of the raw $\hlo$ correlation ($\rho = 0.72$) and the product predictor $\hlo \times A_{L_1}^\text{base}$ ($\rho = 0.86$).
Table~\ref{tab:loo} reports leave-one-out (LOO) Spearman correlations for $\hlo$ alone.

\begin{table}[h]
\caption{Leave-one-out sensitivity analysis for the $\hlo$ scaling trend ($k = 7$). WOS and CLINC have the highest Cook's distances (3.76 and 0.94), reflecting their roles as boundary cases: WOS as a floor-effect dataset and CLINC as the strongest effect. Dropping WOS improves $\rho$ to 0.78 ($p = 0.066$), and the product predictor $\hlo \times A_{L_1}^\text{base}$ resolves this without dropping any dataset ($\rho = 0.86$, $p = 0.014$).}
\label{tab:loo}
\centering
\small
\begin{tabular}{lccccc}
...
\midrule
None (full) & 7 & 0.72 & 0.068 & 0.49 & 0.261 \\
\midrule
Yahoo & 6 & 0.55 & 0.257 & 0.42 & 0.412 \\
GoEmotions & 6 & 0.60 & 0.208 & 0.46 & 0.363 \\
Newsgroups & 6 & 0.71 & 0.111 & 0.48 & 0.331 \\
TREC & 6 & 0.84 & 0.036 & 0.50 & 0.317 \\
arXiv & 6 & 0.81 & 0.050 & 0.50 & 0.314 \\
CLINC & 6 & 0.67 & 0.148 & 0.49 & 0.322 \\
WOS & 6 & 0.78 & 0.066 & 0.90 & 0.014 \\
\bottomrule
\end{tabular}
\end{table}

The raw $\hlo$ correlation is influenced by WOS (Cook's $D = 3.76$), which has the deepest hierarchy but the lowest $L_1$ learnability.
Dropping WOS improves $\rho$ to $0.78$ ($p = 0.066$), and the product predictor $\hlo \times A_{L_1}^\text{base}$ resolves this without dropping any dataset ($\rho = 0.86$, $p = 0.014$; see Section~\ref{sec:scaling}).
A bootstrap analysis (10{,}000 resamples) of the raw $\hlo$ correlation yields 96.4\% of values $> 0$.
The meta-analysis prediction interval for a new dataset's Cohen's $d$ is $[-0.70, 3.18]$, reflecting heterogeneity ($I^2 = 53\%$) that the interaction model predicts: datasets differ systematically in effect size based on both hierarchy depth and learnability.
