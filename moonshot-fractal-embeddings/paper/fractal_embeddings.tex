...
On CLINC-150, V5 achieves $\steer = +0.144 \pm 0.031$ while MRL produces $\steer = +0.015 \pm 0.011$ ($p < 0.01$).
Causal ablations identify alignment as the driver: within a matched protocol, aligned supervision yields positive steerability, inverted alignment reverses direction ($\steer = -0.018$), and removing prefix-specific supervision collapses the signal ($\steer = +0.009$); $p < 10^{-6}$ across 5~seeds.
...
At full embedding length ($j=4$, 256d), V5 and MRL achieve comparable classification accuracy on Yahoo, Newsgroups, and TREC (Table~\ref{tab:accuracy}).
...
\begin{table}[t]
\caption{Classification accuracy at full embedding length ($j = 4$, 256d). V5 and MRL are comparable on Yahoo, Newsgroups, and TREC. On CLINC, V5 retains higher $L_0$ while MRL retains higher $L_1$---reflecting V5's coarse-to-fine allocation. Both improve substantially over the unfinetuned baseline.}
\label{tab:accuracy}
\centering
\small
...
CLINC & 0.961 & 0.955 & 0.913 & 0.889 & 0.681 & 0.713 \\
\bottomrule
\end{tabular}
\end{table}
...
Steerability increases monotonically with hierarchy complexity ($\hlo$): from $\steer = +0.011$ on Yahoo ($\hlo = 1.23$) to $\steer = +0.144$ on CLINC ($\hlo = 3.90$).
...
CLINC & 3.90 & $+0.144 \pm 0.031$ & $+0.015 \pm 0.011$ & $+0.129$ & 3 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\caption{Causal ablation on CLINC-150 (5 seeds, BGE-small). All conditions share a fixed train/val split to eliminate data-split variance (reducing absolute $\steer$ vs.\ Table~\ref{tab:steerability} but tightening within-condition variance). Inverting alignment reverses the sign; removing prefix-specific alignment collapses it. All differences significant at $p < 10^{-6}$.}
\label{tab:ablation}
\centering
\small
\begin{tabular}{lcccc}
...
\midrule
CLINC ($\hlo = 3.90$) & $+0.144 \pm 0.031$ & $+0.153 \pm 0.013$ & $+0.015 \pm 0.011$ & $+0.008 \pm 0.006$ \\
