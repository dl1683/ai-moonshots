...
At full embedding length ($j=4$, 256d), V5 and MRL achieve comparable classification accuracy on Yahoo, GoEmotions, Newsgroups, TREC, arXiv, DBPedia Classes, and WOS (Table~\ref{tab:accuracy}).
...
\begin{table}[t]
\caption{Classification accuracy at full embedding length ($j = 4$, 256d). V5 and MRL are comparable on all eight datasets. On CLINC, V5 retains higher $L_0$ while MRL retains higher $L_1$---reflecting V5's coarse-to-fine allocation. Both improve substantially over the unfinetuned baseline.}
\label{tab:accuracy}
\centering
\small
...
After Holm--Bonferroni correction across eight tests, CLINC, DBPedia Classes, and TREC remain significant ($p_\text{adj} < 0.001$, $< 0.001$, and $0.012$; Cohen's $d = 6.2$, $8.7$, and $2.8$); arXiv and WOS are borderline ($p_\text{adj} = 0.064$ and $0.078$).
...
The substantial heterogeneity ($I^2 = 63\%$) is expected: the scaling trend analysis (Section~\ref{sec:scaling}) predicts that effect size is moderated by the interaction of hierarchy depth and model learnability.
...
The meta-analysis prediction interval for a new dataset's Cohen's $d$ is $[-0.70, 3.18]$, reflecting heterogeneity ($I^2 = 63\%$) that the interaction model predicts: datasets differ systematically in effect size based on both hierarchy depth and learnability.
