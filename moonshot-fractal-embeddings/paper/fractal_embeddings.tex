...
The formal reduction to standard SR proceeds as follows.
Standard SR of $Y_1$ under log loss (both stages targeting $Y_1$) requires Stage~1 rate $R_1 \geq I(\mathbf{z}_{\leq 1}; Y_1)$ and total rate $R_1 + R_2 \geq I(\mathbf{z}; Y_1)$.
By the data processing inequality applied to the Markov chain $\mathbf{z}_{\leq 1} - Y_1 - Y_0$ (since $Y_0 = g(Y_1)$), we have $I(\mathbf{z}_{\leq 1}; Y_0) \leq I(\mathbf{z}_{\leq 1}; Y_1)$.
V5's Stage~1 targets $Y_0$ and thus requires rate $R_1' \geq I(\mathbf{z}_{\leq 1}; Y_0) \leq R_1$---a \emph{strictly weaker} constraint than standard SR.
Since No~\citep{no2019universality} guarantees that standard SR is achievable under log loss for any discrete memoryless source, V5's relaxed regime is achievable a fortiori: targeting a coarser quantity at Stage~1 cannot make the problem harder.
