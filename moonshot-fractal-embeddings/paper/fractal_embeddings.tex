...
The key finding is in the \emph{scaling law at fixed capacity}: steerability at $d_s = 16$ scales nearly perfectly with conditional entropy across all four datasets ($\rho = 1.000$, $r = 0.989$, $p = 0.011$), confirming that hierarchy complexity drives the steerability signal at matched capacity.
...
Confirmed: MRL with $4.4\times$ more parameters ($9.2$M vs.\ $2.1$M) yields $\steer = +0.003$ on CLINC ($d = 12.9$ vs.\ V5 head-only).
