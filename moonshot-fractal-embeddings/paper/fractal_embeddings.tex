...
\begin{itemize}
    \item \textbf{Shallow hierarchies}: On Yahoo Answers ($K_0 = 4$, $\hlo = 1.23$), steerability is small and noisy ($\steer = 0.011 \pm 0.010$). The method is most valuable for deep hierarchies with many coarse classes.
    ...
    \item \textbf{Two-level hierarchies}: Our current formulation handles two-level hierarchies ($L_0$, $L_1$). Extension to deeper hierarchies (3+ levels) is straightforward in principle but untested.
    \item \textbf{Single-seed cross-model}: Qwen3 results are currently from a single seed; multi-seed replication is in progress.
\end{itemize}
