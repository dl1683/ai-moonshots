...
Across nine hierarchical datasets, steerability improvements are consistent and large (meta-analytic $d = 1.49$, $p < 0.0001$; sign test $9/9$, $p = 0.002$), and four causal ablations confirm the mechanism ($p_\text{adj} < 10^{-3}$, $d \geq 6.1$).
A pre-registered product-form scaling predictor identifies \emph{when} the method works best ($\rho = 0.85$, $p = 0.004$; out-of-sample prediction error 4.3\%).
...
\begin{enumerate}
    ...
    \item \textbf{Causal and cross-dataset evidence}: nine datasets, meta-analytic $d = 1.49$ ($p < 0.0001$), sign test $9/9$ ($p = 0.002$), and four ablations confirming alignment as the mechanism ($d \geq 6.1$; Section~\ref{sec:causal}).
    \item \textbf{A pre-registered scaling predictor} linking steerability to hierarchy depth $\times$ model learnability ($\rho = 0.85$, $p = 0.004$; out-of-sample error 4.3\%), with a Goldilocks capacity--demand optimum ($R^2 = 0.964$; Section~\ref{sec:scaling}).
    \item \textbf{Broad generality}: three encoder families, million-scale retrieval (BEIR, 4.6M documents), natural deep taxonomies (HUPD patents), and $3.7\times$ HNSW query speedups (Section~\ref{sec:generality}).
\end{enumerate}
