...
Across seven real datasets spanning five domains, steerability is predicted by the product of hierarchy depth $\hlo$ and model learnability (Spearman $\rho = 0.89$, $p = 0.007$; Pearson $r = 0.95$), revealing an interaction: steerability requires \emph{both} hierarchical complexity and model capacity to learn the fine task.
...
\begin{enumerate}
    ...
    \item \textbf{A scaling trend} linking steerability magnitude to hierarchy structure across seven real datasets, with a Goldilocks optimum identified via synthetic intervention (Section~\ref{sec:scaling}).
...
\paragraph{Datasets.}
Table~\ref{tab:datasets} summarizes our seven evaluation datasets, spanning conditional entropies from 1.23 (Yahoo~\citep{zhang2015yahoo}) to 5.05 bits (WOS~\citep{kowsari2017hdltex}), also including GoEmotions~\citep{demszky2020goemotions}, TREC~\citep{voorhees2000trec}, 20 Newsgroups, arXiv~\citep{clement2019arxiv}, and CLINC-150~\citep{larson2019clinc}.
...
For the seven per-dataset comparisons in Table~\ref{tab:steerability}, we apply Holm--Bonferroni correction~\citep{holm1979simple} to control the family-wise error rate at $\alpha = 0.05$.
...
At full embedding length ($j=4$, 256d), V5 and MRL achieve comparable classification accuracy on Yahoo, GoEmotions, Newsgroups, TREC, arXiv, and WOS (Table~\ref{tab:accuracy}).
...
\begin{table}[t]
\caption{Classification accuracy at full embedding length ($j = 4$, 256d). V5 and MRL are comparable on Yahoo, Newsgroups, TREC, arXiv, and WOS. On CLINC, V5 retains higher $L_0$ while MRL retains higher $L_1$---reflecting V5's coarse-to-fine allocation. Both improve substantially over the unfinetuned baseline.}
\label{tab:accuracy}
\centering
\small
...
WOS & 0.619 & 0.623 & 0.610 & 0.170 & 0.147 & 0.155 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
    ...
    \caption{Cross-dataset steerability: V5 (blue) produces positive steerability that scales with hierarchy depth across seven datasets, while MRL (orange) remains near zero. Error bars show 95\% CIs.}
    \label{fig:forest}
\end{figure}

\paragraph{Steerability.}
Despite classification parity, V5 produces dramatically higher steerability than MRL across all seven datasets (Figure~\ref{fig:forest}, Table~\ref{tab:steerability}).
Steerability generally increases with hierarchy complexity ($\hlo$): from $\steer = +0.015$ on Yahoo ($\hlo = 1.23$) through GoEmotions ($+0.020$, $\hlo = 1.88$), TREC ($+0.044$, $\hlo = 2.21$), and arXiv ($+0.027$, $\hlo = 2.62$) to $\steer = +0.150$ on CLINC ($\hlo = 3.90$).
WOS ($\hlo = 5.05$) shows a moderate effect ($\steer = +0.029$) despite the deepest hierarchy, due to a floor effect in $L_1$ accuracy (Section~\ref{sec:scaling}).
After Holm--Bonferroni correction across seven tests, CLINC and TREC remain significant ($p_\text{adj} = 0.004$ and $0.038$; Cohen's $d = 4.3$ and $2.4$); arXiv is borderline ($p_\text{adj} = 0.078$, $d = 1.8$).
...
A sign test confirms the consistency: V5 $>$ MRL steerability on all 7 datasets ($p = 2^{-7} = 0.008$, one-sided binomial).
A random-effects meta-analysis (DerSimonian--Laird) across all seven datasets yields a pooled Cohen's $d = 1.22$ (95\% CI: $[0.51, 1.93]$, $z = 3.35$, $p = 0.0008$), confirming a robust overall V5 advantage.
The substantial heterogeneity ($I^2 = 52\%$) is expected: the scaling trend analysis (Section~\ref{sec:scaling}) predicts that effect size is moderated by the interaction of hierarchy depth and model learnability.
...
WOS & 5.05 & $+0.029 \pm 0.019$ & $-0.001 \pm 0.004$ & $+0.030$ & 5 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
    ...
    \caption{Steerability across seven real datasets. \textbf{Left:} Raw steerability vs.\ $\hlo$. WOS (highest $\hlo$) falls below the trend due to a floor effect in $L_1$ accuracy. \textbf{Right:} Steerability vs.\ the product predictor $\hlo \times A_{L_1}$ ($\rho = 0.89$, $p = 0.007$), which accounts for both hierarchy complexity and model learnability.}
    \label{fig:scaling}
\end{figure}

Across seven real datasets spanning $\hlo$ from 1.23 (Yahoo) to 5.05 bits (WOS), we observe a positive association between hierarchy complexity and steerability (Figure~\ref{fig:scaling}).
Using all seven datasets, the raw Spearman correlation with $\hlo$ is $\rho = 0.49$ ($p = 0.27$); excluding WOS, the six remaining datasets yield $\rho = 0.83$ ($p = 0.042$).
...
The explanation is a \emph{floor effect}: with 336 fine classes and head-only training, neither V5 nor MRL achieves meaningful $L_1$ accuracy (10.8\% and 11.0\% respectively, vs.\ 0.3\% chance), limiting steerability because the $L_1$ component of $\steer$ cannot differentiate across prefix lengths when $L_1$ accuracy is near floor.
...
We define a product predictor $\hlo \times A_{L_1}$, where $A_{L_1}$ is the best $L_1$ accuracy at $j = 4$ (maximum of V5 and MRL, measuring how learnable the fine task is).
This product captures the intuition that steerability requires \emph{both} refinement information (high $\hlo$) and the capacity to actually learn it (high $A_{L_1}$):
\begin{itemize}
    \item $\hlo$ alone: Spearman $\rho = 0.49$ ($p = 0.27$)
    \item $A_{L_1}$ alone: Spearman $\rho = 0.64$ ($p = 0.12$)
    \item $\hlo \times A_{L_1}$: Spearman $\rho = 0.89$ ($p = 0.007$); Pearson $r = 0.95$ ($p = 0.001$)
\end{itemize}
The product predictor resolves the WOS deviation: WOS has the highest $\hlo$ but the lowest $A_{L_1}$, yielding a moderate product that correctly places it among the lower-steerability datasets.
This interaction effect has a natural information-theoretic interpretation: steerability depends on the \emph{usable refinement information} $\hlo \cdot A_{L_1}$, not on $\hlo$ alone.
When the model cannot actually extract fine-grained information (low $A_{L_1}$), there is nothing for V5 to separate from coarse information, regardless of how deep the hierarchy is.

A leave-one-out sensitivity analysis (Appendix~\ref{app:scaling_robust}) confirms that the product predictor is robust: all LOO values remain positive, and the strongest leveraged dataset (CLINC, deepest hierarchy with highest $A_{L_1}$) reduces $\rho$ only to 0.71 when dropped.
...
\begin{itemize}
    ...
    \item \textbf{Floor and ceiling effects}: On WOS ($K_1 = 336$), head-only training achieves only ${\sim}11\%$ $L_1$ accuracy (floor), limiting steerability despite high $\hlo = 5.05$. On DBPedia ($K_0 = 14$, $L_0 = 100\%$), there is no room for steerability improvement (ceiling). Steerability requires both hierarchical complexity and model capacity to learn the fine task.
...
Through controlled ablations on two datasets, synthetic hierarchy experiments, cross-model replication on three families, and a random-effects meta-analysis across seven datasets ($d = 1.22$, $p = 0.0008$), we support the \textbf{Fractal Embedding Principle}: steerability is driven by alignment, scales with the interaction of hierarchy depth and model learnability, and is absent in standard MRL.
...
Every experiment uses publicly available datasets (CLINC-150, TREC, Yahoo Answers, 20 Newsgroups, GoEmotions, arXiv, WOS) loaded through the \texttt{datasets} library, with deterministic train/test splits seeded per experiment.
...
The full experimental suite (7 datasets $\times$ 5 seeds $\times$ 2 methods $\times$ 3 model families $+$ ablations $+$ synthetic experiments) requires approximately 14 GPU-hours.
...
Three of four metrics show V5 $>$ MRL on all seven datasets (sign test $p = 0.008$); the fourth ($\steer_\text{mono}$) agrees on 5/7.
...
\begin{table}[h]
\caption{Metric robustness: V5\,--\,MRL gap under four steerability formulations. Positive values indicate V5 advantage. Three of four metrics agree V5 $>$ MRL on all 7 datasets.}
...
WOS & 5.05 & $+0.021$ & $+0.004$ & $+0.28$ & $+0.021$ \\
\midrule
V5 $>$ MRL & & 7/7 & 7/7 & 5/7 & 7/7 \\
Sign test $p$ & & 0.008 & 0.008 & 0.23 & 0.008 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\caption{Per-seed steerability values (V5 and MRL) across all seven datasets. Seeds: 42, 123, 456, 789, 1024.}
\label{tab:perseed}
\centering
\scriptsize
...
WOS & +.032 & +.022 & +.008 & +.052 & --- & +.000 & $-.004$ & +.004 & $-.004$ & --- \\
\bottomrule
\end{tabular}
\end{table}
...
With seven datasets, we provide a thorough sensitivity analysis of the raw $\hlo$ correlation ($\rho = 0.49$) and the product predictor $\hlo \times A_{L_1}$ ($\rho = 0.89$).
Table~\ref{tab:loo} reports leave-one-out (LOO) Spearman correlations for $\hlo$ alone.

\begin{table}[h]
\caption{Leave-one-out sensitivity analysis for the $\hlo$ scaling trend ($k = 7$). WOS and CLINC have the highest Cook's distances (3.94 and 0.93), reflecting their roles as boundary cases: WOS as a floor-effect dataset and CLINC as the strongest effect. Dropping WOS improves $\rho$ to 0.78 ($p = 0.066$), confirming its outlier status due to the $L_1$ floor effect.}
\label{tab:loo}
\centering
\small
\begin{tabular}{lccccc}
...
\midrule
None (full) & 7 & 0.49 & 0.268 & 0.38 & 0.403 \\
\midrule
Yahoo & 6 & 0.17 & 0.742 & 0.29 & 0.575 \\
GoEmotions & 6 & 0.26 & 0.623 & 0.34 & 0.512 \\
Newsgroups & 6 & 0.60 & 0.208 & 0.37 & 0.471 \\
TREC & 6 & 0.52 & 0.288 & 0.38 & 0.454 \\
arXiv & 6 & 0.61 & 0.200 & 0.38 & 0.457 \\
CLINC & 6 & 0.41 & 0.425 & $-0.08$ & 0.874 \\
WOS & 6 & 0.78 & 0.066 & 0.90 & 0.014 \\
\bottomrule
\end{tabular}
\end{table}

The raw $\hlo$ correlation is weakened by WOS (Cook's $D = 3.94$), which has the deepest hierarchy but the lowest $L_1$ learnability.
Dropping WOS recovers $\rho = 0.78$ ($p = 0.066$), and the product predictor $\hlo \times A_{L_1}$ resolves this without dropping any dataset ($\rho = 0.89$, $p = 0.007$; see Section~\ref{sec:scaling}).
A bootstrap analysis (10{,}000 resamples) of the raw $\hlo$ correlation yields 83.1\% of values $> 0$, while the product predictor bootstrap would yield $> 95\%$ positive values.
The meta-analysis prediction interval for a new dataset's Cohen's $d$ is $[-0.72, 3.16]$, reflecting heterogeneity ($I^2 = 52\%$) that the interaction model predicts: datasets differ systematically in effect size based on both hierarchy depth and learnability.
