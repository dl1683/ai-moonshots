...
On CLINC-150, V5 achieves $\steer = +0.150 \pm 0.028$ vs.\ MRL $\steer = +0.007 \pm 0.016$ ($p_\text{adj} = 0.004$, Cohen's $d = 4.3$, 5~seeds).
Controlled ablations identify alignment as the causal driver: inverting alignment reverses the steerability sign, removing prefix supervision collapses it, and a uniform multi-task baseline (hierarchy-aware but not aligned) produces near-zero steerability; all comparisons significant ($p \leq 0.03$, $d \geq 2.7$).
A synthetic hierarchy experiment reveals a Goldilocks optimum where steerability peaks when coarse task complexity matches prefix capacity (quadratic $R^2 = 0.964$).
Across eight real datasets spanning six domains, steerability is predicted by the product of hierarchy depth $\hlo$ and baseline learnability (Spearman $\rho = 0.90$, $p = 0.002$; Pearson $r = 0.97$).
Cross-model replication on three encoder families (BGE-small, E5-small, Qwen3-0.6B) confirms architecture invariance.
We connect these findings to the classical theory of successive refinement, providing formal information-theoretic grounding for \emph{why} and \emph{when} hierarchy-aligned supervision produces steerability.
\end{abstract}
...
After Holm--Bonferroni correction across eight tests, DBPedia Classes, CLINC, and TREC remain significant ($p_\text{adj} = 0.002$, $0.004$, and $0.038$; paired Cohen's $d = 5.5$, $4.3$, and $2.4$); arXiv is borderline ($p_\text{adj} = 0.078$) and WOS does not reach significance ($p_\text{adj} = 0.111$).
...
The stronger evidence for semantic separation comes from the steerability metric itself (\S\ref{sec:results}), which measures the \emph{operational} consequence of prefix truncation rather than the raw information content.
...
To verify architecture invariance, we replicate the CLINC experiment on two additional model families: E5-small-v2~(Microsoft, contrastive pre-training, 384d) and Qwen3-Embedding-0.6B ($h = 1024$, $18\times$ more parameters than BGE-small), plus TREC on Qwen3.
...
V5 adaptive (64d for coarse, 256d for fine) achieves $L_0 = 97.5\%$ and $L_1 = 94.5\%$---\emph{higher} coarse accuracy at one-quarter the dimensionality, with comparable fine accuracy, using a single model and single index.
...
The 3-level steerability $\steer_{02} = +0.027 \pm 0.005$ (V5) vs.\ $+0.002 \pm 0.005$ (MRL, 3~seeds; paired $t = 18.9$, $p = 0.003$, Cohen's $d = 10.9$), confirming that fractal supervision generalizes to deeper hierarchies with strong statistical significance even at $n = 3$.
...
\begin{itemize}
    ...
    \item \textbf{Hierarchy noise robustness}: To test sensitivity to imperfect hierarchies, we corrupt $L_0$ labels at rates from 10\% to 50\% and retrain V5 on CLINC (3~seeds).
    Steerability degrades gracefully: at 10\% noise, 85\% of steerability is retained ($\steer = +0.152$); at 30\%, 71\% ($\steer = +0.128$); even at 50\% corruption, steerability remains positive ($\steer = +0.061$, 34\% retention).
    This confirms that V5 does not require a perfect hierarchy to produce meaningful steerability.
    \item \textbf{Baseline scope}: We compare against a matched MRL baseline that controls all training variables except the prefix-to-label mapping.
    We do not benchmark against HEAL~\citep{zhang2025heal}, CSR~\citep{chen2025csr}, or SMRL~\citep{li2025smrl} because these methods address orthogonal goals: HEAL optimizes hierarchy-aware \emph{global} embeddings without prefix steerability; CSR targets sparse compression, not hierarchical granularity; SMRL improves MRL's fidelity ramp but retains flat supervision.
    None offers a prefix-level steering mechanism, making accuracy comparisons at fixed dimensionality inapposite---the relevant comparison is the \emph{existence} of steerability, which our ablations and the UHMT control isolate.
\end{itemize}
