...
Across five real datasets, steerability is perfectly rank-ordered by conditional entropy $\hlo$ (Spearman $\rho = 1.0$, $p < 0.001$).
...
\paragraph{Datasets.}
Table~\ref{tab:datasets} summarizes our five evaluation datasets, spanning conditional entropies from 1.23 (Yahoo~\citep{zhang2015yahoo}) to 3.90 bits (CLINC-150~\citep{larson2019clinc}), also including GoEmotions~\citep{demszky2020goemotions}, TREC~\citep{voorhees2000trec}, and 20 Newsgroups.
...
At full embedding length ($j=4$, 256d), V5 and MRL achieve comparable classification accuracy on Yahoo, GoEmotions, Newsgroups, and TREC (Table~\ref{tab:accuracy}).
...
\paragraph{Steerability.}
Despite classification parity, V5 produces dramatically higher steerability than MRL across all five datasets (Figure~\ref{fig:forest}, Table~\ref{tab:steerability}).
Steerability increases monotonically with hierarchy complexity ($\hlo$): from $\steer = +0.015$ on Yahoo ($\hlo = 1.23$) through GoEmotions ($+0.020$, $\hlo = 1.88$) and TREC ($+0.045$, $\hlo = 2.21$) to $\steer = +0.144$ on CLINC ($\hlo = 3.90$).
...
\begin{figure}[t]
    ...
    \caption{Steerability scales with hierarchy refinement entropy $\hlo$ across five real datasets (Spearman $\rho = 1.0$, $p < 0.001$).}
    \label{fig:scaling}
\end{figure}

Across five real datasets, steerability is perfectly rank-ordered by conditional entropy $\hlo$ (Spearman $\rho = 1.0$, $p < 0.001$; Figure~\ref{fig:scaling}).
A linear fit yields $\steer \approx 0.053 \cdot \hlo - 0.068$ ($R^2 = 0.95$).
