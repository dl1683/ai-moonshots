...
HEAL achieves ${\sim}3\times$ higher mean steerability than V5 (overall paired $t = 10.2$, $p < 10^{-6}$).
...
Normalising steerability by training compute, V5 achieves $9\times$ higher steerability per GFLOP (Figure~\ref{fig:pareto}).
...
\begin{figure}[t]
    ...
    \caption{Efficiency--steerability Pareto frontier. \textbf{Left}: Steerability vs.\ head parameters. \textbf{Right}: Steerability vs.\ training compute. V5 (blue circles) achieves $9\times$ higher steerability per GFLOP than HEAL (green diamonds). Each point is one dataset. CSR and SMEC cluster near zero steerability regardless of compute.}
    \label{fig:pareto}
\end{figure}
...
CLINC & 3.90 & $+0.275 \pm 0.012$ & $+0.150 \pm 0.028$ & $+0.007 \pm 0.016$ & $+0.003$ & $-0.002$ \\
\bottomrule
\end{tabular}
\end{table}
