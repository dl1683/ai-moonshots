...
A backbone fine-tuning control on two datasets confirms that alignment is both necessary and sufficient: a flat MRL baseline with $4.4\times$ more trainable parameters (backbone fine-tuning) produces zero steerability ($d = 8.1$ and $d = 4.8$ vs.\ head-only V5 on CLINC and DBPedia Classes respectively).
Replication across three encoder families confirms architecture invariance.
We ground these findings in the classical theory of successive refinement: exploiting the log-loss universality theorem, we prove that hierarchy-aligned prefix supervision is compatible with optimal fine encoding, and derive the Goldilocks optimum and scaling law from information-theoretic first principles.
\end{abstract}
...
A key enabling result is the \emph{log-loss universality theorem}~\citep{kostina2019universality}: any discrete source is successively refinable when the coarse stage uses cross-entropy, which is exactly V5's prefix loss.
Section~\ref{sec:theory} exploits this to prove that hierarchy-aligned prefix supervision is compatible with optimal fine encoding, and derives testable predictions---sign reversal under inversion, a Goldilocks capacity--demand optimum, and a product scaling law---all confirmed by our experiments.
...
On CLINC: V5-frozen $\steer = +0.050 \pm 0.005$ vs.\ MRL+backbone $\steer = +0.003 \pm 0.003$ ($d = 8.1$, $p < 10^{-4}$).
On DBPedia Classes: V5-frozen $\steer = +0.053 \pm 0.010$ vs.\ MRL+backbone $\steer = -0.000 \pm 0.003$ ($d = 4.8$, $p < 10^{-3}$; Figure~\ref{fig:backbone}).
On both datasets, the alignment factor has a massive effect; the capacity factor has none.
Specifically, $4.4\times$ more trainable parameters cannot recover steerability without hierarchy-aligned prefix supervision ($p = 0.38$ and $p = 0.71$ for MRL-frozen vs.\ MRL+backbone on CLINC and DBPedia respectively).
Adding backbone fine-tuning to V5 yields no improvement on CLINC ($p = 0.62$) but \emph{reduces} steerability on DBPedia Classes: V5+backbone $\steer = +0.035 \pm 0.006$ vs.\ V5-frozen $+0.053 \pm 0.010$ ($d = 2.7$, $p = 0.004$).
...
Alignment remains both necessary and sufficient: head-only V5 produces higher steerability than V5 with $4.4\times$ more parameters.
...
Crucially, Kostina and Tuncel~\citep{kostina2019universality} proved a \emph{universality} result: any discrete memoryless source is successively refinable when the first-stage decoder uses \emph{logarithmic loss}---which is mathematically identical to cross-entropy.
...
\end{equation}
\emph{Moreover, this prefix allocation is compatible with optimal fine encoding: the coarse prefix does not constrain the achievable rate for $Y_1|Y_0$ in the remaining dimensions.
...
The key non-triviality is that this does not penalise the fine stage.
Since the prefix loss is cross-entropy (log loss) and $Y_0$ is a deterministic function of $Y_1$, the log-loss universality theorem~\citep{kostina2019universality} guarantees successive refinability: the remaining dimensions $\mathbf{z}_{2:J}$ can achieve the optimal rate for $Y_1|Y_0$ without rate loss.
...
A factorial backbone control on two datasets confirms alignment is both necessary and sufficient: $4.4\times$ more trainable parameters without alignment produce zero steerability ($d = 8.1$ on CLINC, $d = 4.8$ on DBPedia Classes).
\emph{Theoretically}, the successive refinement framework---anchored by the log-loss universality theorem---explains \emph{why} V5 works (cross-entropy at the prefix guarantees compatibility with optimal fine encoding) and \emph{when} it works best (Goldilocks capacity--demand matching, $R^2 = 0.964$).
