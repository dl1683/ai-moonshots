"""
Real-World Benchmark: 20 Newsgroups
===================================

20 Newsgroups has a natural 2-level hierarchy:
- 6 super-categories: comp, sci, rec, talk, soc, misc
- 20 sub-categories

This tests fractal vs flat on REAL text data.
"""
...
class NewsGroupsDataset(Dataset):
    """20 Newsgroups with 2-level hierarchy."""

    def __init__(self, split: str = "train", max_samples: int = 5000, seed: int = 42):
        random.seed(seed)
        np.random.seed(seed)

        print(f"Loading 20 Newsgroups ({split})...")
        ...
        for item in ds:
            ...
            text = item['text'][:512]  # Truncate
            label_text = item['label_text']

            # Find super-category
            super_cat = None
            for sc, subs in HIERARCHY.items():
                if label_text in subs:
                    super_cat = sc
                    break

            if super_cat is None:
                continue  # Skip if not in our hierarchy
    ...
    def num_classes_per_level(self) -> List[int]:
        return [len(HIERARCHY), len(SUB_TO_IDX)]
...
def run_single_seed(seed, backbone, tokenizer, device, epochs=8, batch_size=32):
    ...
    train_data = NewsGroupsDataset(split="train", max_samples=8000, seed=seed)
    val_data = NewsGroupsDataset(split="test", max_samples=2000, seed=seed + 1000)
...
def main():
    ...
    # Run experiments
    print("\n" + "="*60)
    print("20 NEWSGROUPS BENCHMARK (2-level hierarchy)")
    print("6 super-categories, 20 sub-categories")
    print("="*60)
    ...
    # Save results
    results_path = Path(__file__).parent.parent / "results" / f"newsgroups_benchmark_{args.model}.json"
    with open(results_path, "w") as f:
        json.dump({
            'dataset': '20_newsgroups',
            'hierarchy': '6 super-categories, 20 sub-categories',
            'model': args.model,
            'seeds': args.seeds,
            'statistics': stats,
