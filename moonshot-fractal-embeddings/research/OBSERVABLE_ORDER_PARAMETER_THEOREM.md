...
2. **Anisotropic covariance**: Theorems proven for sigma^2*I. Real neural networks
   have anisotropic within-class covariance (low eta). Extension requires:
   - Correction factor 1/sqrt(eta) in the logit formula
   - Finite-eta corrections to Gumbel parameters

3. **Cross-task universality**: kappa/sqrt(K) is good but imperfect for cross-dataset
   (Qwen2/TREC MAE=0.098). Better K-normalization needed.
...
5. **External replication**: All results from this repo. Independent replication
